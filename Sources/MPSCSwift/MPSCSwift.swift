...
public class FIFOQueue<T> {
    public var fifoQueue = [T]()
    public var queueID: String {
        get {
            //return String(format: "%08x", ObjectIdentifier(self).hashValue)
            let objIdentifierString = ObjectIdentifier(self).debugDescription
            let start = objIdentifierString.index(objIdentifierString.startIndex, offsetBy: 25)
            let end = objIdentifierString.index(objIdentifierString.endIndex, offsetBy: -1)
            let range = start..<end
            
            return String(objIdentifierString[range])
        }
    }
}
extension FIFOQueue: CustomStringConvertible {
    public var description: String {
        return "FIFOQueue<\(self.queueID): \(self.fifoQueue.count) items>"
    }
}
public class Packet<T> {
    /* This version uses a simple fifoQueue managed using DispatchQueue.sync */
    var fifoQueue: FIFOQueue<T>
    let dispatchQueueForSync: DispatchQueue
    let dispatchSemaphore: DispatchSemaphore
    
    public init() {
        self.fifoQueue = FIFOQueue()
        self.dispatchQueueForSync = DispatchQueue(label: UUID().uuidString)
        self.dispatchSemaphore = DispatchSemaphore(value: 0)
    }
    private init(fifoQueue: FIFOQueue<T>, dispatchQueueForSync: DispatchQueue, dispatchSemaphore: DispatchSemaphore) {
        self.fifoQueue = fifoQueue
        self.dispatchQueueForSync = dispatchQueueForSync
        self.dispatchSemaphore = dispatchSemaphore
    }
    public func clone() -> Packet<T> {
        return Packet(
            fifoQueue: self.fifoQueue,
            dispatchQueueForSync: self.dispatchQueueForSync,
            dispatchSemaphore: self.dispatchSemaphore
        )
    }
    public func send(t: T) {
        self.dispatchQueueForSync.sync {
            self.fifoQueue.fifoQueue.append(t)
            //+print("DBG_PRINT: Item added to fifo queue \(self.fifoQueue).")
            self.dispatchSemaphore.signal()
            //+print("DBG_PRINT: Dispatch semaphore signaled")
        }
    }
    public func tryRecv() -> Result<T, TryRecvError> {
        var result: T? = nil
        self.dispatchQueueForSync.sync {
            if !self.fifoQueue.fifoQueue.isEmpty {
                result = self.fifoQueue.fifoQueue.removeFirst()
                //+print("DBG_PRINT: tryRecv(), Item removed from fifo queue \(self.fifoQueue)")
            } else {
                //+print("DBG_PRINT: tryRecv(), fifo queue \(self.fifoQueue.queueID) is empty.")
            }
        }
        if let result = result {
            return .success(result)
        } else {
            return .failure(TryRecvError.Empty)
        }
    }
    public func recv(timeout: Duration? = nil) -> Result<T, TryRecvError> {
        if let timeout = timeout {
            let endtime = DispatchTime.now() + DispatchTimeInterval.milliseconds(Int(timeout))
            let _ = self.dispatchSemaphore.wait(timeout: endtime)
        } else {
    ...
    // For debugging:
    public var queueID: String { get { return self.fifoQueue.queueID }}
}
...
public class SendError<T>: Error {
    public let t: T
    public init(t: T) {
        self.t = t
    }
...
public class RecvError: Error { public init(){}}
//
...
public enum TryRecvError: Error {
...
public enum RecvTimeoutError: Error {
    // /// This **channel** is currently empty, but the **Sender**(s) have not yet
    // /// disconnected, so data may yet become available.
    // #[stable(feature = "mpsc_recv_timeout", since = "1.12.0")]
    // Timeout,
...
//
// enum Flavor<T> {
//public enum Flavor<T> {
    // Oneshot(Arc<oneshot::Packet<T>>),
    //case oneshot(Packet<T>)
    // Stream(Arc<stream::Packet<T>>),
    //case stream(Packet<T>)
    // Shared(Arc<shared::Packet<T>>),
    //case shared(Packet<T>)
...
public func channel<T>(channelType: T.Type) -> (Sender<T>, Receiver<T>) {
    // let a = Arc::new(oneshot::Packet::new());
    let a = Packet<T>()
    let b = a.clone()
    // (Sender::new(Flavor::Oneshot(a.clone())), Receiver::new(Flavor::Oneshot(a)))
    return (Sender(inner: b), Receiver(inner: a))
}
// }
//
...
public func syncChannel<T>(channelType: T.Type, bound: Int) -> (SyncSender<T>, Receiver<T>) {
    // let a = Arc::new(sync::Packet::new(bound));
    let a = Packet<T>()
    let b = a.clone()
    // (SyncSender::new(a.clone()), Receiver::new(Flavor::Sync(a)))
    return (SyncSender(inner: b), Receiver(inner: a))
}
// }
//
// ////////////////////////////////////////////////////////////////////////////////
...
// impl<T> Clone for Sender<T> {
    ...
    public func clone() -> Sender<T> {
        return Sender(inner: self.inner)
    ...
    // }
    // For debugging:
    public var queueID: String { get { return self.inner.queueID }}
}
// }
//
// #[stable(feature = "rust1", since = "1.0.0")]
...
// impl<T> SyncSender<T> {
    ...
    // pub fn send(&self, t: T) -> Result<(), SendError<T>> {
    public func send(t: T) throws {
        //+print("DBG_PRINT: In SyncSender.send().")
...
// impl<T> Receiver<T> {
    ...
    public func tryRecv() throws -> T {
        return try self.inner.tryRecv().get()
        // loop {
    ...
    public func recv() -> Result<T, RecvError> {
        switch inner.recv() {
        case .success(let result): return .success(result)
        case .failure(_): return .failure(RecvError())
        }
        // loop {
        //     let new_port = match *unsafe { self.inner() } {
        //         Flavor::Oneshot(ref p) => {
    ...
    public func recvTimeout(timeout: Duration) -> Result<T, RecvTimeoutError> {
        //+print("DBG_PRINT in recvTimeout(\(timeout)) for queue \(self.inner.fifoQueue.description)")
        return self.inner.recv(timeout: timeout).mapError({ (tryRecvError) -> RecvTimeoutError in
            switch tryRecvError {
            case .Disconnected: return RecvTimeoutError.Disconnected
            case .Empty: return RecvTimeoutError.Timeout
            }
        })
