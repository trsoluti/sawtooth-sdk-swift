...
public enum Update {
    ...
    case PeerMessage(PeerMessage, PeerId)
    case BlockNew(EngineBlock)
    case BlockValid(BlockId)
    case BlockInvalid(BlockId)
    case BlockCommit(BlockId)
    case Shutdown
}
//
...
// pub struct Block {
public struct EngineBlock {
    // pub block_id: BlockId,
    public let blockID: BlockId
    // pub previous_id: BlockId,
    public let previousBlockID: BlockId
    // pub signer_id: PeerId,
    public let signerID: PeerId
    // pub block_num: u64,
    public let blockNum: UInt64
    // pub payload: Vec<u8>,
    public let payload: [UInt8]
    // pub summary: Vec<u8>,
    public let summary: [UInt8]
}
extension EngineBlock: DefaultValueProtocol {
    public typealias T = EngineBlock
    public static var defaultValue: EngineBlock =
        EngineBlock(
            blockID: BlockId([]),
...
extension PeerInfo: DefaultValueProtocol {
    public typealias T = PeerInfo
    public static var defaultValue: PeerInfo = PeerInfo(peerID: PeerId.defaultValue)
}
// }
...
public struct PeerMessage {
    ...
    public let header: PeerMessageHeader
    // pub header_bytes: Vec<u8>,
    public let headerBytes: [UInt8]
    // pub header_signature: Vec<u8>,
    public let headerSignature: [UInt8]
    // pub content: Vec<u8>,
    public let content: [UInt8]
}
extension PeerMessage:DefaultValueProtocol {
    public static var defaultValue: PeerMessage = PeerMessage(
        header: PeerMessageHeader.defaultValue,
        headerBytes: [],
        headerSignature: [],
        content: [])
}
...
public struct PeerMessageHeader {
    ...
    public let signerID: [UInt8]
    // pub content_sha512: Vec<u8>,
    public let contentSHA512: [UInt8]
...
extension PeerMessageHeader: DefaultValueProtocol {
    public static var defaultValue: PeerMessageHeader =
        PeerMessageHeader(
            signerID: [],
            contentSHA512: [],
            messageType: "",
...
public protocol Engine {
    ...
    var additionalProtocols: [(String, String)] { get }
}
// }
//
...
public struct StartupState {
    ...
    public let chainHead: EngineBlock
    // pub peers: Vec<PeerInfo>,
    public let peers: [PeerInfo]
    // pub local_peer_info: PeerInfo,
    public let localPeerInfo: PeerInfo
}
extension StartupState: DefaultValueProtocol {
    public static var defaultValue: StartupState =
        StartupState(
            chainHead: EngineBlock.defaultValue,
            peers: [],
            localPeerInfo: PeerInfo(peerID: PeerId.defaultValue)
    )
}
// }
//
// #[derive(Debug)]
// pub enum Error {
public enum ConsensusError: Error {
